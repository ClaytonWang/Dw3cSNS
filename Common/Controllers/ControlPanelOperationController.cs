        ...
        public ActionResult MassMessages(bool isByRole, List<string> roleName, bool isMessage, string body, string subject, int minRank = 0, int maxRank = 0)
        {
            ...
            else
            {
                if (isByRole)
                {
                    if (roleName.Count > 0)
                    {
                        roleName[0] = roleName[0].TrimEnd(',');
                        users = UserService.GetUsers(roleName, 0, 0);
                        if (users.Count()==0)
                        {
                            return Json(new StatusMessageData(StatusMessageType.Error, string.Format("{0}发送失败了,请选择单个管理员！", isMessage ? "消息" : "邮件")));
                        }
                    }
                }
